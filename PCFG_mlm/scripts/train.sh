...
save_freq=1000   #<-- frequency of saving checkpoint
...
subdir_log_name=$subdir"_mlm_lr"$lr"_hid"$hid_size"_nL"$nlayers"_samples"$subsample"_nH"$n_heads"_seed"$seed"_bt"${BATCH_SIZE}"_accsteps"${ACCU_STEPS}'_'${pcfg_type}
OUTPUT_DIR=$MODEL_DIR"/output/test/"$subdir_log_name
LOGGING_DIR=$MODEL_DIR"/output/logs/"$subdir_log_name
  

header="python $RUN_DIR/main_pcfg_mlm.py"
${header}  \
    --do_train True \
    ...
    --mask_token_id 6\
    --cls_token_id 5\
    --project_name 'pcfg_mlm_bert_80_10_10_'${pcfg_type};
