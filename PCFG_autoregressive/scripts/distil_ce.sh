...
PARENT_MODEL='openai-community/gpt2'
data_seed=0
...
speedfactor=2        #<-- Speedfactor; For a teacher checkpoint at T steps, we supervise the student with T/speedfactor steps; 
                     #set as 1000 for one-shot distillation
refresh_frequency=4  # < -- the refresh frequency for changing the teacher, set as 200 (checkpoints selected will be 400,800,1200,1600,2000,...)
...
ckpt_idx='4'        #<-- index of first checkpoint
REF_CKPT_DIR=$RUN_DIR'/output/test/_ce_lr5e-3_hid256_nL4_samples1_024_000_nH32_seed0_bt8_accsteps64_yuanzhi_cfg3f' #<-- teacher checkpoint 
...
OUTPUT_DIR=$RUN_DIR"/output/test_"${subsample}"/"$pcfg_type"_pcfg_ce_distil_1epoch_lr"${lr}"_hid"${hid_size}"_nlayers"${nlayers}"_temp"${temp}"_speedfactor"${speedfactor}"_refreshfreq"${refresh_frequency}"_teacher"$teacher_hidsize"hidnlayers"$teacher_nlayers'_pcfgtype'$pcfg_type'_seed'${seed}'_'${pcfg_type}
LOGGING_DIR=$RUN_DIR"/output/logs_"${subsample}"/"$pcfg_type"_pcfg_ce_distil_1epoch_lr"${lr}"_hid"${hid_size}"_nlayers"${nlayers}"_temp"${temp}"_speedfactor"${speedfactor}"_refreshfreq"${refresh_frequency}"_teacher"$teacher_hidsize"hidnlayers"$teacher_nlayers'_pcfgtype'$pcfg_type'_seed'${seed}'_'${pcfg_type}

python $RUN_DIR/distil_pcfg_ce.py \
    --do_train True \
    ...
    --mask_token_id 6\
    --cls_token_id 5\
    --project_name 'distil_PCFG_ce_'${pcfg_type}'_'${subsample}'_temp'${temp};
